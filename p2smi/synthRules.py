...
def check_forbidden_motifs(seq):
    return [desc for desc, pattern in forbidden_motifs.items() if re.search(pattern, seq)]
...
def check_terminal_residues(seq):
    return "C-terminal residue is Pro or Cys, which may cause synthesis issues" if seq[-1] in ["P", "C"] else None

def check_glycine_runs(seq):
    return "More than 4 consecutive glycines found" if re.search(r"G{4,}", seq) else None
...
def collect_synthesis_issues(smiles, seq):
    ...
    for check_fn in [check_cysteine_content, check_terminal_residues, check_glycine_runs, check_length]:
        result = check_fn(seq)
        if result:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate peptide synthesis feasibility from file.")
    parser.add_argument("-i", "--input_file", required=True, help="Input file with peptide-smiles lines")
    parser.add_argument("-o", "--output_file", help="Optional output file to write results")
