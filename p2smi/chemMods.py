...
def add_n_methylation(sequence, methylation_residue_fraction):
    ...
    for pos in sorted(random.sample(positions, min(num_to_methylate, len(positions))), reverse=True):
...
def process_sequences(input_lines, nmeth_rate, peg_rate, nmeth_residues):
    ...
    methylate_indices = set(random.sample(range(total), math.ceil(total * nmeth_rate))) if nmeth_rate > 0 else set()
    pegylate_indices = set(random.sample(range(total), math.ceil(total * peg_rate))) if peg_rate > 0 else set()
    ...
    for i, (header, seq) in enumerate(parse_input_lines(input_lines)):
        ...
        sequence, mods = modify_sequence(seq, i in methylate_indices, i in pegylate_indices, nmeth_residues)
...
def main():
    parser = argparse.ArgumentParser(description="Modify peptide sequences with PEGylation and N-methylation.")
    ...
    parser.add_argument("--peg_rate", type=float, default=0.2, help="Fraction of sequences to PEGylate (0-1).")
    parser.add_argument("--nmeth_rate", type=float, default=0.2, help="Fraction of sequences to N-methylate (0-1).")
    parser.add_argument("--nmeth_residues", type=float, default=0.2, help="Fraction of bonds in each sequence to N-methylate (0-1).")
