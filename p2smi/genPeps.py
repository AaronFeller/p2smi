...
def get_amino_acid_lists():
    ...
    upper_noncanonical = [aa for aa in all_aas if aa not in canonical and aa not in lower_canon and aa.isupper()]
    lower_noncanonical = [aa for aa in all_aas if aa not in canonical and aa not in lower_canon and aa.islower()]
...
def build_sequence(seq_len, noncanonical_percent, dextro_percent, amino_lists):
    ...
    ln_c, dn_c, ln_nc, dn_nc = calculate_amino_acid_counts(seq_len, noncanonical_percent, dextro_percent)
    sequence_parts = (
...
def generate_sequences(num_sequences, min_length, max_length, noncanonical_percent, dextro_percent, constraints):
...
def main():
    ...
    constraints = CONSTRAINTS if args.constraints == ['all'] or not args.constraints else args.constraints
