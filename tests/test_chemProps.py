...
def test_log_partition_coefficient_valid():
    assert round(log_partition_coefficient("CCN(CC)C(=O)[C@H]1CN([C@@H]2CC3=CNC4=CC=CC(=C34)C2=C1)C"), 2) == 2.91  # ethanol approx
...
def test_lipinski_trial_pass_and_fail():
    ...
    passed, failed = lipinski_trial("CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC")  # very large
